...
public class GridLayoutManager extends SeslLinearLayoutManager {
    ...
    @Override
    public int getRowCountForAccessibility(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state) {
    ...
    @Override
    public int getColumnCountForAccessibility(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state) {
    ...
    @Override
    public void onInitializeAccessibilityNodeInfoForItem(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, View host, AccessibilityNodeInfoCompat info) {
    ...
    @Override
    void onAnchorReady(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, AnchorInfo anchorInfo, int itemDirection) {
    ...
    private void ensureAnchorIsInCorrectSpan(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, AnchorInfo anchorInfo, int itemDirection) {
    ...
    View findReferenceChild(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, int start, int end, int itemCount) {
        ...
        for (int i = start; i != end; i += diff) {
            ...
            if (position >= 0 && position < itemCount) {
                ...
                if (((SeslRecyclerView.LayoutParams) view.getLayoutParams()).isItemRemoved()) {
    ...
    private int getSpanGroupIndex(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, int viewPosition) {
        if (!state.isPreLayout()) {
    ...
    private int getSpanIndex(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, int pos) {
        if (!state.isPreLayout()) {
    ...
    private int getSpanSize(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, int pos) {
        if (!state.isPreLayout()) {
    ...
    @Override
    void collectPrefetchPositionsForLayoutState(SeslRecyclerView.State state, SeslLinearLayoutManager.LayoutState layoutState, LayoutPrefetchRegistry layoutPrefetchRegistry) {
    ...
    @Override
    void layoutChunk(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, SeslLinearLayoutManager.LayoutState layoutState, LayoutChunkResult result) {
    ...
    private void measureChildWithDecorationsAndMargin(View child, int widthSpec, int heightSpec, boolean alreadyMeasured) {
        SeslRecyclerView.LayoutParams lp = (SeslRecyclerView.LayoutParams) child.getLayoutParams();
    ...
    private void assignSpans(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, int count, boolean layingOutInPrimaryDirection) {
    ...
    private int computeScrollRangeWithSpanInfo(SeslRecyclerView.State state) {
    ...
    private int computeScrollOffsetWithSpanInfo(SeslRecyclerView.State state) {
        if (getChildCount() == 0 || state.getItemCount() == 0) {
            return 0;
        }
        ensureLayoutState();
