...
public class DatePicker extends LinearLayout implements SeslSimpleMonthView.OnDayClickListener, View.OnClickListener, View.OnLongClickListener, SeslSimpleMonthView.OnDeactivatedDayClickListener {
    ...
    private View.OnClickListener mCalendarHeaderClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            setCurrentViewType((mCurrentViewType + DATE_MODE_START) % DATE_MODE_END);
        }
    };
    ...
    public interface OnEditTextModeChangedListener {
        void onEditTextModeChanged(DatePicker view, boolean edit);
    }

    public interface OnViewTypeChangedListener {
        void onViewTypeChanged(DatePicker view);
    }
    ...
    public DatePicker(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        ((LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(Build.VERSION.SDK_INT >= 23 ? R.layout.sesl_date_picker : R.layout.sesl_date_picker_legacy, this, true);
        ...
        mSpinnerLayout.setOnSpinnerDateChangedListener(this, new SeslDatePickerSpinnerLayout.OnSpinnerDateChangedListener() {
            @Override
            public void onDateChanged(SeslDatePickerSpinnerLayout view, int year, int monthOfYear, int dayOfMonth) {
                ...
                if (mMode == DATE_MODE_WEEK_SELECT && mIsWeekRangeSet) {
                    updateStartEndDateRange(getDayOffset(), year, monthOfYear, dayOfMonth);
                }
    ...
    private void updateSimpleMonthView(boolean smoothScroll) {
        ...
        this.mCalendarViewPager.setCurrentItem(var3, smoothScroll);
    ...
    private class CalendarPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(@NonNull View container, int position) {
            ...
            simpleMonthView.setOnDayClickListener(DatePicker.this);
            simpleMonthView.setOnDeactivatedDayClickListener(DatePicker.this);
        ...
        if (getLayoutParams().height == ViewGroup.LayoutParams.WRAP_CONTENT || getMeasuredHeight() <= mDatePickerHeight) {
            ...
            if (mContentFrame != null) {
                ...
                if (mDialogWindow != null) {
                    contentHeight -= mDialogPaddingVertical;
                }
            }
    ...
    private void updateViewType(int height) {
        ...
        if (Build.VERSION.SDK_INT >= 24 && activity != null && activity.isInMultiWindowMode()) {
            if (height >= mDatePickerHeight && !mCalendarHeaderText.hasOnClickListeners()) {
                mCalendarHeaderText.setOnClickListener(mCalendarHeaderClickListener);
    ...
    public void setDialogPaddingVertical(int padding) {
        mDialogPaddingVertical = padding;
    }
    ...
    private class DayOfTheWeekView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            int i2;
            super.onDraw(canvas);
            if (DatePicker.this.mNumDays != 0) {
                int i3 = (DatePicker.this.mDayOfTheWeekLayoutHeight * 2) / 3;
                int i4 = DatePicker.this.mDayOfTheWeekLayoutWidth / (DatePicker.this.mNumDays * 2);
                for (int i5 = 0; i5 < DatePicker.this.mNumDays; i5++) {
                    char charAt = this.mWeekdayFeatureString.charAt(i5);
                    int i6 = (i5 + 2) % DatePicker.this.mNumDays;
                    if (charAt == 'B') {
                ...
                for (int i7 = 0; i7 < DatePicker.this.mNumDays; i7++) {
                    int i8 = (DatePicker.this.mWeekStart + i7) % DatePicker.this.mNumDays;
                    this.mDayLabelCalendar.set(7, i8);
                    String upperCase = DatePicker.this.mDayFormatter.format(this.mDayLabelCalendar.getTime()).toUpperCase();
                    if (DatePicker.this.mIsRTL) {
                        i2 = ((((DatePicker.this.mNumDays - 1) - i7) * 2) + 1) * i4;
                        i = DatePicker.this.mPadding;
                    } else {
                        i2 = ((i7 * 2) + 1) * i4;
                        i = DatePicker.this.mPadding;
                    }
            ...
            case VIEW_TYPE_SPINNER:
                ...
                if (mCurrentViewType != type) {
                    switch (mMode) {
                        case DATE_MODE_START:
                            if (mIsLunar) {
                                mSpinnerLayout.updateDate(mLunarStartYear, mLunarStartMonth, mLunarStartDay);
                            } else {
                                mSpinnerLayout.updateDate(mStartDate.get(Calendar.YEAR), mStartDate.get(Calendar.MONTH), mStartDate.get(Calendar.DAY_OF_MONTH));
                            }
                        ...
                        case DATE_MODE_END:
                            if (mIsLunar) {
                                mSpinnerLayout.updateDate(mLunarEndYear, mLunarEndMonth, mLunarEndDay);
                            } else {
                                mSpinnerLayout.updateDate(mEndDate.get(Calendar.YEAR), mEndDate.get(Calendar.MONTH), mEndDate.get(Calendar.DAY_OF_MONTH));
                            }
                            mAnimator.setDisplayedChild(1);
                            mSpinnerLayout.setEnabled(true);
                            mCurrentViewType = type;
                        ...
                        default:
                            if (mIsLunar) {
                                mSpinnerLayout.updateDate(mLunarCurrentYear, mLunarCurrentMonth, mLunarCurrentDay);
                            } else {
                                mSpinnerLayout.updateDate(mCurrentDate.get(Calendar.YEAR), mCurrentDate.get(Calendar.MONTH), mCurrentDate.get(Calendar.DAY_OF_MONTH));
                            }
                            mAnimator.setDisplayedChild(1);
                            mSpinnerLayout.setEnabled(true);
                            mCurrentViewType = type;
    ...
    public void setEditTextMode(boolean edit) {
        if (mCurrentViewType != VIEW_TYPE_CALENDAR) {
            mSpinnerLayout.setEditTextMode(edit);
        }
    }

    public boolean isEditTextMode() {
        return mCurrentViewType != VIEW_TYPE_CALENDAR && mSpinnerLayout.isEditTextMode();
    }

    public void setOnEditTextModeChangedListener(OnEditTextModeChangedListener listener) {
        mSpinnerLayout.setOnEditTextModeChangedListener(this, listener);
    }

    public EditText getEditText(int index) {
        return mSpinnerLayout.getEditText(index);
    }

    public NumberPicker getNumberPicker(int index) {
        return mSpinnerLayout.getNumberPicker(index);
    }
            ...
            if (mClassLoader == null) {
                try {
                    ...
                    if (sourceDir == null || TextUtils.isEmpty(sourceDir)) {
                        Log.e(DatePicker.TAG, "getPathClassLoader, calendar package source directory is null or empty");
                        return null;
                    }

                    mClassLoader = new PathClassLoader(sourceDir, ClassLoader.getSystemClassLoader());
                } catch (PackageManager.NameNotFoundException unused) {
                    Log.e(DatePicker.TAG, "getPathClassLoader, calendar package name not found");
                    return null;
                }
            }
